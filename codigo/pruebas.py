...
asignaturas.sort(key= lambda asig: (
                                    asig['semestre'] if asig['semestre'] is not None else float('inf'),
...
for asignatura in asignaturas:
    semestre_asignatura = asignatura['semestre'] if asignatura['semestre'] is not None else -float('inf')
...
for i in range(1,max_sem+1):
    ...
    with plan_estudios.subgraph(name=f'cluster_{i}',
        ...
        plan_estudios.node(name=f'sem_{i}',
                width='0.02',
                height='0.02',
                style='invis')
        ...
        if ((not en_curso) or (max_sem != i))  \
            and (semestre_asignatura != -float('inf')):
            PA = 5
